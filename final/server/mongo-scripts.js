...
mongo "mongodb+srv://admin:NRYhjC22srRcGmHV@battery-marketplace.g9e2h.mongodb.net/myFirstDatabase?retryWrites=true&w=majority" --username admin
...
db.listings.find({description: {$regex: "Prime"}, model: "Prius"}).forEach(function (listing) {
  listing.model = "Prius Prime"
  db.listings.save(listing)
})

db.listings.find({model: {$ne: {$regex: "Hybrid"}}}).map(listing => print(listing.model))
db.listings.find({model: {$regex: "Fusion"}}).map(listing => print(listing.model))
...
generations.Toyota.forEach(generation => {
  ...
  db.listings.find({make: "Toyota", model: genModel, year: {$gte: genFirst, $lte: genLast}}).forEach(listing => {
...
Object.keys(sampleModuleGenerations).map(makeKey => {    
    db.listings.find({make: makeKey, module: "module_here"}).forEach(listing => {
      listing.module = ""
    })
  })
...
Object.keys(moduleGenerations).map(makeKey => { print(makeKey) })
...
db.listings.find({}).forEach(listing => {
  ...
  if (make == "Hyundai") {
    print("Hyundai")
    const module = db.modules.find({make: "Kia", models: {$in: [model]}, firstYear: {$lte: year}, lastYear: { $gte: year}})[0]
  } else {
    const module = db.modules.find({make: make, models: {$in: [model]}, firstYear: {$lte: year}, lastYear: { $gte: year}})[0]
  }
...
db.listings.updateMany({model: "MKZ" },
